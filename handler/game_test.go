...
func TestCheckGuess(t *testing.T) {
	...
	received := make([]service.GuessResults, 0, len(testcases))
	mockTemplates.EXPECT().Render(
		mock.Anything,
		mock.AnythingOfType("string"),
		mock.Anything,
		...
		guessResults, ok := data.(service.GuessResults)
		if !ok {
			t.Errorf("data is not service.GuessResults")
		}
		received = append(received, guessResults)
		return nil
	})

	// i set playerId and answer to the same value so i mock this easier
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			fmt.Println("received:", received[i])

			assert.Equal(t, tc.expectedCode, w.Code)
			if tc.expectedCode >= 400 {
				assert.Contains(t, w.Body.String(), tc.expectedMsg)
			} else {
				assert.Equal(t, tc.expectedResult, received[i].Rows[0].Result)
				i++
			}
		})
	}
